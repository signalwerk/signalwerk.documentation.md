...
const tokenize = (charDelim, charEsc, str) => {
  const [token, list, _] = str.split("").reduce(
    ([aToken, aList, aEsc], x) => {
      const blnBreak = !aEsc && x === charDelim, blnEscChar = !aEsc && x === charEsc;
...
Handlebars.registerHelper("example", function (text, options) {
  let markers = tokenize(",", "\\", options.hash.marker).map((item) => item.split("=").map((item) => item.trim())
  );
  ...
  markers.forEach((item, i) => {
    ...
    title = title
      .replace(
        search,
        `<span class="mark mark--${i} mark--${item[2] || "normal"}">${item[1]}</span>`
      )
      .replace("\\n", `<br />`);
    code = code
      .replace("\\n", "")
      ...
      .replace(
        search,
        `</code><code class="mark mark--${i} mark--${item[2] || "normal"}">&ZeroWidthSpace;${Handlebars.escapeExpression(item[1])}</code><code>`
      );
  });
